...
def multiclass_auc(trues, preds):
    ...
    :param preds: Predicted probabilities
    :return: Average AUC
    trues is n by 3 # n is the number of objects
    '''
    nClasses = trues.shape[1]
    
    # Identify the splits between each class in the data
    splits = np.sum(trues, axis=0, dtype='int') # the number of 1's in each column
...
def generate_confusion_matrix(trues, preds, classes, title):
    '''
    ...
    confusionMatrix = metrics.confusion_matrix(new_trues, new_preds)
    print(title)
    plot_confusion_matrix(confusionMatrix, classes, title)